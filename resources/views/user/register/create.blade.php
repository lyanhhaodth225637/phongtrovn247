...
@section('content')
    ...
    .verify-state-icon.success { background: #f0fdf4; color: #15803d; }
    .verify-state-icon.warning { background: #fffbeb; color: #d97706; }
    .verify-state-title {
      font-family: 'Sora', sans-serif;
      font-size: 1.05rem;
      font-weight: 800;
    ...
    .verify-state-badge.success { background: #f0fdf4; color: #15803d; border: 1px solid #bbf7d0; }
    .verify-state-badge.warning { background: #fffbeb; color: #92400e; border: 1px solid #fde68a; }
    ...
    .verify-alert.success { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; }
    .verify-alert.success i { color: #16a34a; margin-top: 1px; flex-shrink: 0; }
    .verify-alert.danger  { background: #fff1f2; color: #9f1239; border: 1px solid #fecdd3; }
    .verify-alert.danger  i { color: #e02424; margin-top: 1px; flex-shrink: 0; }
    ...
    .verify-input.is-invalid  { border-color: #e02424; }
    .verify-input:read-only   { background: #f8fafc; color: var(--muted); cursor: default; }
    ...
    .verify-otp-row .verify-input-wrap { flex: 1; }
    .btn-send-otp {
      display: inline-flex;
      align-items: center;
      gap: 5px;
    ...
    .btn-send-otp:hover:not(:disabled) { background: var(--primary-light); border-color: var(--primary); }
    .btn-send-otp:disabled { color: var(--muted); border-color: var(--border); cursor: not-allowed; opacity: .7; }
    ...
    .verify-security i { color: #d97706; flex-shrink: 0; margin-top: 1px; }

    @media (max-width: 480px) {
      .verify-card-body { padding: 20px 18px 24px; }
      .verify-card-head { padding: 22px 18px 20px; }
      .verify-otp-row { flex-direction: column; }
      .btn-send-otp { width: 100%; justify-content: center; }
    }
    </style>
          ...
          @if($emailVerified && $isApproved)
            ...
            <div class="verify-state">
              ...
              <div class="verify-state-badge success">
                <i class="bi bi-check-circle-fill"></i> Tài khoản đã được duyệt
              </div>
              <div class="verify-state-title">Chào mừng bạn trở lại!</div>
              <div class="verify-state-sub">
                Tài khoản nhà cho thuê của bạn đã được xác thực và phê duyệt đầy đủ.<br>
                Bạn có thể sử dụng toàn bộ tính năng của nền tảng.
              </div>
              <a href="{{ route('frontend.home') }}" class="btn-home">
                <i class="bi bi-house-fill"></i> Vào Trang Chủ
              </a>
            </div>

          @elseif($emailVerified && !$isApproved)
            {{-- ĐÃ XÁC THỰC EMAIL NHƯNG CHƯA ĐƯỢC DUYỆT --}}
            <div class="verify-state">
              ...
              <div class="verify-security">
                ...
                Nếu cần hỗ trợ, hãy liên hệ hotline <strong>1900 6868</strong> hoặc gửi email tới
                <strong>support@phongtrovn247.com</strong>
              </div>
            </div>

          @else
            {{-- CHƯA XÁC THỰC EMAIL → HIỂN THỊ FORM OTP --}}
                ...
                <div class="verify-otp-row">
                  <div class="verify-input-wrap">
                    ...
                    <input type="text" name="otp"
                      class="verify-input @error('otp') is-invalid @enderror"
